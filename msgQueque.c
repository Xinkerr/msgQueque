...
{
	return ringbuffer_init(msg, msg_pool, pool_size);
}
 ...
 *			   0：成功
 */
int8_t msgQueque_put(msgQueque_t* msg, uint8_t *pdata, msgbuf_szie_t size)
{
    if(size == 0)
        return 0;
	//有空间、无异常下加入消息队列数据
    if(ringbuffer_free_space(msg) >= size+1)
    {
        ringbuffer_put(msg, (uint8_t*)&size, sizeof(msgbuf_szie_t));
        ringbuffer_put(msg, pdata, size);
        return 0;
    }
    else
        return -1;
...
msgbuf_szie_t msgQueque_get(msgQueque_t* msg, uint8_t* msg_content)
{
    ...
    msgbuf_szie_t len = 0;
    if(ringbuffer_get(msg, (uint8_t*)&len, sizeof(len)) == 0)
    {
        ringbuffer_get(msg, msg_content, len);
    }
